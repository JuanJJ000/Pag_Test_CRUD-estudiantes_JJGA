...
form.addEventListener('submit', async (e) => { 
    ...
    if(docId)
    {
        await actualizarEstudiante(docId, estudainteData);   
    }
    else{
        await agregarEstudiante(estudainteData);
    }
    form.reset();
    await init();
});
...
tabla.addEventListener('click', async (e) => {
    if (e.target.classList.contains('btn-editar')) { 
        ...
        idInput.value = docId; // Usamos el input oculto para guardar el documentId
    ...
    estudiantes.forEach (est => {   
        const fila =`
            <tr data-document-id="${est.documentId}">
                <td>${est.id}</td>
                <td>${est.Nombre}</td>
                <td>${est.Apellido}</td> <td>${est.edad}</td>
                <td>
...
async function init() {
const estudiantes = await obtenerEstudiantes(); cargarTabla (estudiantes);
}
init();